...
description: Deep research for topics touching politics or culture — anywhere the left-right axis runs through the evidence ecosystem: elections, policy, crime, immigration, education, public-health controversies, DEI, gender, climate policy, guns, contested history, media and institutional fights, "the real story" of any coded event or organization. Use whenever Landon asks for deep research or a "behind the press releases" account where media framing or Claude's documented left lean could tilt the answer. Five lanes (Advocate-Right with extra effort, Advocate-Left, Primary-Record, Ground-Experience, Reversal Lab), evidence-tier adjudication, labeled counterfactual forecasts, frame verification. Compensating-asymmetry premise: Claude is a left-biased instrument, so the process holds right rudder while evidence standards stay fixed on truth. ROUTER — if honest researchers left and right would converge given the same evidence (products, neutral science/health/tech, travel, business, uncoded history), use deep-research-general instead.
---
