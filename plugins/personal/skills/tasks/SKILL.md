...
Before working the board, **read the ✅ Task System — Operating Model** and follow it. It's a page inside **📍 START HERE** (ClickUp doc `2kydf11b-754`) — list that doc's pages, or use its Index (§9), to find it. The model holds what you can't guess: the Lists and how to route to them, the status set, task formatting, where tasks come from, the board-vs-wiki split, the cadence, and the guardrails. (This skill points at START HERE rather than the model page and hardcodes no other page IDs, so it survives brain restructuring — START HERE owns the routing.)
